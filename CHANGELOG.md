...
- **License: Apache 2.0 → PolyForm Noncommercial 1.0.0.** Free for personal and other noncommercial use. Commercial use, including use inside a business, needs a commercial license from SmartMemory (help@smartmemory.ai). `LICENSE` carries the official PolyForm text under a `Required Notice:` copyright line, `ts/package.json` and `meta/pyproject.toml` use SPDX `PolyForm-Noncommercial-1.0.0`, and `ts/LICENSE` is added to the npm `files` list so the published package carries the terms (it previously shipped `dist` only, with no license file). Releases before this change stay Apache 2.0.
