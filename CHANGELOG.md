...
- **STRAT-LEARN-INLINE-TS-1 (stratum side):** automatic lesson staging behind `[learn] inline` / `STRATUM_LEARN_INLINE` (default OFF). Every terminal run (completed, failed, cancelled, budget_exhausted, gate completion) triggers a coalesced, fail-open pass over the engine's own store that stages durable lessons for every enabled, existing canonical workspace, honours retire/dismiss suppression, never applies, and logs one diagnostic row per pass to `<store>/../learn-inline/triggers.jsonl`. Surfacing: `unreviewedLessons()`, `stratum learn list --unreviewed [--json] [--if-enabled]` (prints the switch's winning layer), and `stratum_audit.learn_inline` forwarded through MCP. `resolveLearnConfig()` now resolves `inline` alongside `deliver`. MCP surface 25 / events 6: `stratum_audit` declares `learn_inline?`, and `ready` / `fanout_item_ready` / `fanout_item_dispatched` declare the DELIVER-1 `lessons?` / `lessonsSuppressed?` detail fields (without this, delivery ON made `stratum_audit` and `stratum_flow_poll` fail the strict event contract over MCP). The Compose build-summary slice is not yet built.
...
- **STRAT-AGENT-PEER-3: `SendMessage` can reach a running Codex background run (opt-in).** Background Codex runs used `codex exec`, which has no input channel, so the peer sidecar always refused inbound messages (and `codex queue` against a running exec returns rc 0 while silently dropping the message). `STRATUM_CODEX_BG_STRATEGY=app-server` now runs background Codex turns through `codex app-server`: a detached driver owns the app-server, and a `SendMessage` to the run's peer is steered into the active turn (`turn/steer`). Delivery is truthful (`delivered` only after a returned turn id, otherwise dropped/unknown, never retried), steer frames need first-frame token auth, messages to finished runs are refused, and a failed message never ends the run. Sandbox settings go through `thread/start` with the same keys `exec` uses, so ambient Codex config (including temp exclusions) applies identically, and the strategy is never wider than `exec`. `on-failure` approval is rejected before spawn. The protocol is pinned to Codex CLI 0.155.1, and an unsupported version fails the run at the initial handshake. The default stays `exec`, and foreground runs and `STRATUM_CODEX_TRANSPORT` are unchanged. Live exit gate (real Codex 0.155.1, real Claude Code session, gpt-6-luna/low): sandbox 7/7, process tree 8/8, golden flow passed, 3 server-request methods proven live and 8 recorded as not reachable on 0.155.1 (see `docs/features/STRAT-AGENT-PEER-3/report.md`).

- **Claude telemetry reports the dispatched effort.** `ClaudeConnector` passed `effort` to the SDK but its telemetry carried only `durationMs` and `model`, so compose's routing ledger could never attribute an executed tier to a Claude dispatch (all 24 real observations `unknown`). Telemetry now includes `effort` whenever one was handed to the SDK — on success and on failure after dispatch — matching the Codex connector's standard (the dispatched value via `modelIdentity`). Not yet covered: background Claude runs, whose telemetry is built separately in `connectors/background.ts:804`.
...
- **Codex background runs register as Claude Code peer sessions.** The `peerName` field alone
  indicates registration was attempted; poll for `peer.registered` before subscribing with `notify_when_idle`. Set
