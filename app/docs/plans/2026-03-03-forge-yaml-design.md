# `.forge.yaml` — Agent-Agnostic Process Spec
...
Every Forge pipeline today is Claude Code-specific. Phase ordering, model selection, gate behavior,
...
`.forge.yaml` separates the process from the executor. The spec describes what must happen. A
...
```
.forge.yaml (what)           ← this document defines this
      ↓ compile
connector (how)
...
**Policy modes** (Forge's gate/flag/skip model, unchanged):
...
capability_map:
  ...
  critic:   {agent_type: forge-reviewer,   model: sonnet}
...
## Relationship to Forge Roadmap

| Roadmap Item | How `.forge.yaml` relates |
|---|---|
...
- **Forge server as gate broker** — all connectors emit a gate event to the Forge server
  (SSE/WebSocket); the Gate UI in Vision Surface handles approval; server signals back to
  connector. Agent-agnostic, but requires the Forge server to be in the loop.
- **File-based gate** — connector writes a `.gate` file to the feature workspace; an
  external process (Forge server, CLI) watches for it and writes `.gate.approved` when
  the human approves. Maximally simple, survives any agent.

The Forge-server-as-gate-broker option aligns with the Gate UI (Layer 4) in the
...
If `.forge.yaml` adopts the same syntax, non-Claude Code connectors must either:
...
A simpler alternative: ensures in `.forge.yaml` are expressed as named assertions
