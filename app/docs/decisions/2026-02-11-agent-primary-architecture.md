...
**Date:** 2026-02-11
**Status:** DECIDED
**Context:** Determining the minimum bootstrap configuration for Forge
...
What is the minimum bootstrap configuration to make Forge usable for its own development?
...
**Agent-primary architecture.** The embedded agent panel is the primary write interface. The structured UI (tree, board, graph, detail) is the primary read interface with quick-write for simple interactions. Both backed by the same `.forge/` persistence.

The bootstrap reduces to two hard prerequisites:
1. Wire persistence (UI ↔ `.forge/`)
2. Embed agent panel in the UI

Everything after that is built FROM INSIDE Forge using those two capabilities.

## Rationale

1. **The mechanism compounds.** Every feature Forge needs — onboarding, evaluation templates, policy engine, new views, new connectors — can be built by the agent through the agent panel. Investing in the mechanism has unbounded ROI vs fixed ROI per feature.

2. **Self-modification is the product.** Forge with an embedded agent can extend its own UI, build its own connectors, create other apps. The distinction between "manage the work" and "do the work" is artificial — the same environment hosts both.
...
6. **We're already doing it.** This brainstorming session happened in Claude Code modifying the Forge codebase. The only gap is that the terminal is beside the browser, not inside it.
...
- The Forge UI design is still valid (invariant analysis, 5 views, dark theme, information density)
