...
3. **No review filtering.** Our ralph loop review surfaces everything. Anthropic's forge-reviewer uses confidence scoring (0-100, only report ≥80) to reduce false positives. Our loops would benefit from this.
...
- **Global scope.** Agents and the skill live in `~/.claude/`, not in the Forge project. They're user-level tools.
...
**`forge-explorer.md`** — Codebase analysis specialist
...
**`forge-architect.md`** — Architecture design specialist
- Model: sonnet
...
**`forge-reviewer.md`** — Code review specialist
- Model: sonnet
...
1. Launch 2-3 `forge-architect` agents in parallel with different mandates:
...
Add confidence scoring to the forge-reviewer agent definition. The scoring rubric:
...
### Change 4: Rename to `forge`

Rename the skill from `feature-dev` to `forge`:

1. Move `~/.claude/skills/feature-dev/` → `~/.claude/skills/forge/`
2. Update `name:` in SKILL.md frontmatter
3. Update references in Forge codebase (8 files)
4. Update references in other skills (`bug-fix`)
5. `docs/features/feature-dev-v2/` stays as-is (historical provenance)

**Why `forge`:** Short, branded, unmistakable. The skill IS Forge's core workflow — the structured implementation pipeline. "feature-dev" describes what Anthropic's plugin does (develop a feature in one session). "forge" describes what ours does (the full Forge lifecycle). The rename also eliminates the name collision.
...
|-------|-------------|------------|
| Phase 1: Explore & Design | Main agent explores inline | Launch 2-3 `forge-explorer` agents, read their findings |
| Phase 3: Architecture Doc | Main agent writes one approach | Launch 2-3 `forge-architect` agents with competing mandates |
| Phase 4: Blueprint | Main agent reads files | Launch `forge-explorer` for targeted research |
| Phase 7 step 2: Review loop | `superpowers:requesting-code-review` | Launch `forge-reviewer` with confidence scoring |
