...
| 4 | STRAT-AGENT-PEER-3 | Deliver SendMessage text into running Codex background runs. Add an app-server transport so the peer sidecar can map inbound `user` frames to Codex `turn/steer` instead of replying `refused`. `codex queue` probed 2026-09-25 against a running `codex exec`: accepted, dequeued, never delivered. | COMPLETE |
...
| 4 | STRAT-AGENT-RUN-MODEL-VALIDATE | stratum_agent_run validates model/effort/agent against the runtime allowlist and fails fast naming valid values, instead of spending a dispatch to surface a vendor 400 | COMPLETE |
