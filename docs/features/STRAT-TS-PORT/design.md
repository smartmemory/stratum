...
- [ ] MUST: `run:` subflow execution — the engine executes the referenced flow inline: `with:` values resolve to the subflow's input, subflow `do` steps surface as namespaced ready steps (`<step>/<child>` — `stepDone` accepts the namespaced id), subflow events land in the parent's ordered events log with the namespaced stepId, and the subflow's flow output (validated against its contract) becomes the step output; subflow failure takes the step's normal validation-failure/on_fail path (pinned 2026-07-10: execution was owned by no phase — third orphan after `ensure`/`iterate`; P6's parity flows require it to run). **v1 body restriction:** a subflow body may contain task steps only (`do`/`set` with `when`/`after`/`ensure`/`out`/`attempts`/`on_fail`/`iterate`); `gate`/`fanout`/`run` inside a non-entry flow are validation errors (mirrors non-recursion — nested control surfaces would need namespaced gate/fanout state; revisit post-G1 if a real spec needs one)
