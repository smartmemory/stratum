...
- **S5 design delta (2026-09-25, live sandbox probe):** the row above is wrong for the two temp exclusions. Exec never passes a `-c` for them, so exec **inherits** the ambient `sandbox_workspace_write.exclude_tmpdir_env_var` / `exclude_slash_tmp`. `turn/start.sandboxPolicy` requires both fields (`v2/SandboxPolicy.ts`), so sending it forces `false` and overrides the user. Measured with a config profile that sets both exclusions: exec gave `$TMPDIR`/`/tmp` EPERM, app-server wrote both, so app-server was **wider than exec**. Built-in and ordinary configs matched. **Resolution:** encode the exec knobs at `thread/start` (`sandbox: <SandboxMode>` plus `config: {"sandbox_workspace_write.writable_roots": [...], "sandbox_workspace_write.network_access": <bool>}`, the same keys exec passes with `-c`) and send no `turn/start.sandboxPolicy`. The temp exclusions are then inherited exactly as exec does. Never wider than exec is the invariant. The live `sandbox` probe (all seven cases, including `temp-override`) is the proof, and the fix is unproven until it passes.
...
- [ ] Server-request fixtures: each §4 response **type-checks against the generated binding** for its method, the server accepts it (the request completes), and the run continues or ends. Stall watchdog: a request answered and then 120s of silence → `failed` "unattended request stalled".
  - **Amended 2026-09-25 (controller, after S2 review r1 #3 and the live probe):** real-server acceptance is **required for every method the probe can elicit** on the installed CLI with a supported configuration (`approvalPolicy` `never`/`on-request`/`untrusted`, any sandbox mode, a disposable MCP tool). On 0.155.1 that is proven for `item/commandExecution/requestApproval` and `mcpServer/elicitation/request`. `item/fileChange/requestApproval` gets a dedicated trigger attempt. A method the probe cannot elicit is recorded in `report.md` as **not reachable in supported configurations**, with the trigger that was tried. For those methods the evidence is the generated-type check, the strict fake, and the stall watchdog, which bounds any mis-answer to a `failed` run instead of a hang. The probe must still list unreached methods explicitly, and never report them as passed.
