...
3. Live: rebuild, restart nothing (memory `no-kill-ports`); launch a real `stratum_agent_run(codex, background)` from this session via the **rebuilt dist** (memory `compose-stratum-bin-resolver`: rebuild before live-fire), call `ListAgents`, see `codex-<short>-<runId6> · bg · busy`, `SendMessage(notify_when_idle)`, receive the idle notice, confirm the row disappears after the linger. Note: the live MCP server in this session runs the old code until restarted, so this check may need a fresh session; record the outcome honestly in the report.
