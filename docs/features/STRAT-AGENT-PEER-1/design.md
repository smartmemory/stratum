...
- `type:"control", action:"notify_when_idle"` → **acceptance rule**: a frame is *accepted* when it parses, carries a string `msg_id` and a `from` that passes the callback rule below, and the table has fewer than 32 entries; it is then stored as `{msg_id, from}`. A later accepted frame with the same `from` **replaces** the earlier one (Claude Code's own re-subscription semantics); the replaced subscription is dropped and gets no notice. A frame arriving when the table is full is *rejected*: logged, not stored, no notice. If the run has already finished (terminal, inside the linger), an accepted frame is answered immediately.
...
**Still unverified: admission of our notice into the subscriber's model context.** Two dial-backs (with and without `from_mode` mirrored from the request) were sent to this session's socket with a correlated `orig_msg_id`, and no `[Cross-session idle notice]` reached the model. The most likely cause is policy, not protocol: this session runs with bypass permissions and Claude Code reports that it "holds other inbound peer traffic" for user approval, so a notice from a peer of unknown permission class is shown to the user's transcript rather than to the model. The receiver code (`Jvn` schema, `eAn(orig_msg_id, state, finished_at, detail, from_mode, verifiedPeer)`) accepts the frame shape we send. The Phase 7 live check must therefore be judged by the **user's transcript** (or from a session not in bypass mode); if the notice is not admitted there either, the fallback for callers is `stratum_agent_poll`, and the row visibility (listing + status) still stands.
