...
- `bg_started` response gains `peerName` (present whenever registration was attempted, i.e. not disabled by the kill switch or drift guard). **Review r3 correction:** the originally planned `peer: "pending"` companion field was dropped because the MCP contract grammar has no literal type and `"peer?": "string"` would accept any value; `peerName` alone means "registration attempted, poll for `peer.registered`". `stratum_agent_poll` reads `peer.json` if present and reports `peer: { name, registered: true, pid, sock }`, otherwise `peer: { name, registered: false }` while the sidecar is still starting or failed. Callers treat `peerName` as tentative until `registered: true`, and fall back to `stratum_agent_poll` when `SendMessage(notify_when_idle)` to that name errors (row gone or never appeared). `mcp-surface.json` gains exactly these optional fields (strict contracts, see memory `strict-contract-seams`).
