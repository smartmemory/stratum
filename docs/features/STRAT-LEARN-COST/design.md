...
**Status:** IMPLEMENTED (2026-08-30; design revised after gate rounds 1–4, blueprint corrections C1–C9 folded in, egress made opt-in at implementation review) · **Owner:** stratum · **Extends:** STRAT-TS-LEARN
...
**Server-dispatched calls (corrected at blueprint, C3).** `stratum_agent_run` is *not* run-bound (its request carries no `runId`), so the engine cannot record it; compose reports it like any other dispatch through its funnels. The only engine-owned, run-bound dispatch is the engine's own connector call, whose usage settles through the internal `stepDone` path and therefore gets the legacy receipt. There is no `server_dispatch` source.
...
- **Two funnels, not seven sites (blueprint C1).** Compose has two accounting funnels: `context.recordBuildUsage` (`build.js`) and GSD's `recordTsAgentUsage` (`gsd.js`, reached via `ctx.onUsage`). Both gain `meta` and both report receipts. Surface detection (C2) is `hasTool('stratum_usage_report')` via the MCP SDK's tool listing, cached per connection, failing closed to envelope mode.
- **One hook per funnel.** `context.recordBuildUsage(usage, meta)` (`build.js:2934`) gains an optional `meta = { dispatchId, stepId?, source }` and, when a stratum run is active, also calls `stratum.usageReport(runId, receipt)`. The 20 existing call sites pass `meta`; a call site that cannot name a step passes none (run-level). The local accumulator behavior is unchanged.
...
- **`dispatchId`** = the server-returned dispatch id when the call went through `agentRun` (C4); `randomUUID()` is minted only for paths without one (e.g. a fatal error carrying usage but no id). Carried on the result/error object; a retry of a *failed transport* reuses the id (idempotent), a retry that makes a *new model call* mints a new one.
